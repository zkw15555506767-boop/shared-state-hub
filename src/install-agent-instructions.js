...
const targets = [
  {
    ...
    title: "Codex App Shared State Hub Instructions"
  },
  {
    path: path.join(rootDir, "CLAUDE.md"),
    title: "Claude Code Shared State Hub Instructions"
  }
];

const block = `<!-- shared-state-hub:start -->
...
for (const target of targets) {
  const current = fs.existsSync(target.path) ? fs.readFileSync(target.path, "utf8") : `# ${target.title}\n`;
  const next = upsertBlock(current, block);

  if (current === next) {
    console.log(`No changes needed: ${target.path}`);
    continue;
  }

  if (!apply) {
    console.log(`Would update: ${target.path}`);
    continue;
  }

  if (fs.existsSync(target.path)) {
    fs.writeFileSync(`${target.path}.bak-${new Date().toISOString().replaceAll(/[:.]/g, "-")}`, current);
  }
  fs.writeFileSync(target.path, `${next.trimEnd()}\n`);
  console.log(`Updated: ${target.path}`);
}

if (!apply) {
  console.log("Dry run only. Run `npm run setup:instructions` to apply.");
}

function upsertBlock(content, nextBlock) {
  const pattern = /<!-- shared-state-hub:start -->[\s\S]*?<!-- shared-state-hub:end -->/;
